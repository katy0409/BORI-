const CONFIG_KEY = "bori-v13-supabase-config";
...
function showOnly(id) { ["setupScreen", "authScreen", "appShell"].forEach((x) => $("#" + x).classList.toggle("hidden", x !== id)); }
...
function saveConfig(url, key) { localStorage.setItem(CONFIG_KEY, JSON.stringify({ url: url.trim().replace(/\/$/, ""), key: key.trim() })); }
function clearConfig() { localStorage.removeItem(CONFIG_KEY); location.reload(); }
function initClient() {
  const cfg = getConfig();
  if (!cfg?.url || !cfg?.key || !cfg.key.startsWith("sb_")) return false;
...
async function boot() {
  if (!initClient()) { showOnly("setupScreen"); finishSplash(); return; }
...
$("#saveSetupBtn").addEventListener("click", () => {
  ...
  if (!url.includes("supabase.co") || !key.startsWith("sb_publishable_")) return toast("請確認 Project URL 與 Publishable Key");
  saveConfig(url, key); location.reload();
});
$("#resetCloudConfig").addEventListener("click", clearConfig);
$("#resetConfigBtn").addEventListener("click", () => { if (confirm("要重新設定 Supabase 連線嗎？")) clearConfig(); });
